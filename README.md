...
`modelSelection` records what the Set Model form last submitted (`selection` plus the custom fields when the custom variant was used). It is not read by the daemon — the action's prefill reads it to load the last setup back into the form, so re-opening Set Model shows your existing custom configuration instead of the defaults.
