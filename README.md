...
`modelSelection` records what the Set Model form last submitted: the chosen `selection`, plus a `custom` object carrying that variant's fields when Custom was chosen. Selecting a preset clears `custom`. It is not read by the daemon — the action's prefill reads it to load the last setup back into the form, so re-opening Set Model shows your existing configuration instead of the defaults.
