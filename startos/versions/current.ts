...
export const current = VersionInfo.of({
  version: '1.0.10689:0',
  releaseNotes: {
    en_US: `Updated llama.cpp to build b10689.

A maintenance bump — 59 builds since b10630, with no changes required to your setup.
...
- Correctness fixes in the tensor backend: a Vulkan graph-optimisation pass could drop a dependency between aliased tensors, and transposed convolution produced wrong results on batched input. The graph optimiser can now also be told which allocations must not be reused, which re-enables operation fusions the backends previously had to disable to stay safe.
...
llama.cpp publishes one build per merged commit and does not provide a per-build changelog. Full commit range: https://github.com/ggml-org/llama.cpp/compare/b10630...b10689`,
    es_ES: `Actualiza llama.cpp a la compilación b10689.

Una actualización de mantenimiento: 59 compilaciones desde la b10630, sin cambios necesarios en su configuración.
...
- Correcciones de exactitud en el motor de tensores: una fase de optimización del grafo en Vulkan podía descartar una dependencia entre tensores con alias, y la convolución transpuesta daba resultados erróneos con entradas por lotes. Ahora también se puede indicar al optimizador del grafo qué reservas de memoria no deben reutilizarse, lo que vuelve a habilitar fusiones de operaciones que los motores de cálculo tenían que desactivar por seguridad.
...
llama.cpp publica una compilación por cada commit fusionado y no ofrece un registro de cambios por compilación. Rango completo de commits: https://github.com/ggml-org/llama.cpp/compare/b10630...b10689`,
    de_DE: `Aktualisiert llama.cpp auf Build b10689.

Ein Wartungsupdate – 59 Builds seit b10630, ohne Änderungsbedarf an Ihrer Einrichtung.
...
llama.cpp veröffentlicht einen Build pro zusammengeführtem Commit und stellt kein Änderungsprotokoll je Build bereit. Vollständiger Commit-Bereich: https://github.com/ggml-org/llama.cpp/compare/b10630...b10689`,
    pl_PL: `Aktualizuje llama.cpp do kompilacji b10689.

Aktualizacja konserwacyjna — 59 kompilacji od b10630, bez potrzeby zmian w konfiguracji.
...
- Poprawki poprawności w silniku tensorów: optymalizacja grafu w Vulkanie mogła pominąć zależność między tensorami dzielącymi pamięć, a splot transponowany dawał błędne wyniki dla danych wsadowych. Optymalizatorowi grafu można teraz również wskazać, których przydziałów pamięci nie wolno użyć ponownie, co przywraca łączenie operacji, które silniki obliczeniowe musiały dotąd wyłączać dla bezpieczeństwa.
...
llama.cpp publikuje jedną kompilację na scalony commit i nie udostępnia listy zmian dla poszczególnych kompilacji. Pełny zakres commitów: https://github.com/ggml-org/llama.cpp/compare/b10630...b10689`,
    fr_FR: `Met à jour llama.cpp vers la version b10689.

Une mise à jour de maintenance — 59 versions depuis la b10630, sans changement à apporter à votre installation.
...
- Corrections d'exactitude dans le moteur de tenseurs : une passe d'optimisation du graphe Vulkan pouvait perdre une dépendance entre tenseurs partageant leur mémoire, et la convolution transposée donnait des résultats erronés sur des entrées par lots. L'optimiseur du graphe peut désormais aussi se voir indiquer quelles allocations ne doivent pas être réutilisées, ce qui réactive des fusions d'opérations que les moteurs de calcul devaient jusqu'ici désactiver par précaution.
...
llama.cpp publie une version par commit fusionné et ne fournit pas de journal des modifications par version. Plage complète des commits : https://github.com/ggml-org/llama.cpp/compare/b10630...b10689`,
  },
  migrations: {
    up: async ({ effects }) => {},
    down: IMPOSSIBLE,
